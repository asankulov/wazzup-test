...
describe('JWT helper', () => {
	describe('generate token', () => {
		beforeAll(() => {
			Object.assign(jwt, {
				sign: jest.fn()
						  .mockImplementationOnce((payload, secret, options, callback) => callback(false, testToken))
						  .mockImplementationOnce((payload, secret, options, callback) => callback(true, null))
			});
		});
	...
	describe('verify token', () => {
		beforeAll(() => {
			Object.assign(jwt, {
				verify: jest.fn()
							.mockImplementationOnce((payload, secret, callback) => callback(false, testPayload))
							.mockImplementationOnce((payload, secret, callback) => callback(true, testPayload))
							.mockImplementationOnce((payload, secret, callback) => callback(false, testPayload))
			});
			Object.assign(Session, {
				findOne: jest.fn().mockResolvedValueOnce(true).mockResolvedValueOnce(null)
			});
		});
